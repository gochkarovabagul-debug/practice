...
const useFormValues_1 = require("./useFormValues.cjs");
...
const FormDataConsumer = (props) => {
    ...
    isDirty, }, } = form;
    const formData = (0, useFormValues_1.useFormValues)();
    return (React.createElement(exports.FormDataConsumerView, { formData: formData, ...props }));
};
...
const FormDataConsumerView = (props) => {
    ...
    if (arraySource) {
        const scopedFormData = (0, get_js_1.default)(formData, arraySource);
...
const getArraySource = (source) => source.lastIndexOf('.') > 0 && ArraySourceRegex.test(source)
    ? source
    : undefined;
