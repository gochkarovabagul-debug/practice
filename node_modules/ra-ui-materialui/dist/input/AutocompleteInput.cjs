...
const Offline_1 = require("../Offline.cjs");
const InputHelperText_1 = require("./InputHelperText.cjs");
const sanitizeInputRestProps_1 = require("./sanitizeInputRestProps.cjs");
const defaultFilterOptions = (0, material_1.createFilterOptions)();
/**
...
const AutocompleteInput = (inProps) => {
    ...
    const { id, field, isRequired, fieldState: { error, invalid }, } = (0, ra_core_1.useInput)({
        defaultValue,
        id: idOverride,
        field: fieldOverride,
    ...
    const finalChoices = (0, react_1.useMemo)(() => 
...
If you provided a React element for the optionText prop, you must also provide the matchSuggestion prop (used to match the user input with a choice)`);
    ...
    const { getChoiceText, getChoiceValue, getSuggestions } = (0, ra_core_1.useSuggestions)({
        choices: finalChoices,
        limitChoicesToValue,
        matchSuggestion,
    ...
    const canRenderSuggestions = shouldRenderSuggestions == undefined || // eslint-disable-line eqeqeq
        shouldRenderSuggestions(filterValue);
    const handleOpen = (0, ra_core_1.useEvent)((event) => {
        setIsOpen(true);
        onOpenProp?.(event);
    });
    const handleClose = (0, ra_core_1.useEvent)((event, reason) => {
        setIsOpen(false);
        onCloseProp?.(event, reason);
    });
    const handleChange = (0, ra_core_1.useEvent)((newValue) => {
        if (multiple) {
            if (Array.isArray(newValue)) {
                field.onChange(newValue.map(getChoiceValue), newValue);
            }
            else {
                field.onChange([...(field.value ?? []), getChoiceValue(newValue)], newValue);
            }
        }
        else {
    ...
    const debouncedSetFilter = (0, react_1.useCallback)((0, debounce_js_1.default)(filter => {
        if (setFilter) {
    ...
    const currentValue = (0, react_1.useRef)(field.value);
    (0, react_1.useEffect)(() => {
        if (!(0, isEqual_js_1.default)(currentValue.current, field.value)) {
            currentValue.current = field.value;
            debouncedSetFilter('');
        }
    }, [field.value]); // eslint-disable-line
    const { getCreateItem, handleChange: handleChangeWithCreateSupport, createElement, createId, getOptionDisabled: getOptionDisabledWithCreateSupport, } = (0, ra_core_1.useSupportCreateSuggestion)({
        create,
        createLabel,
        createItemLabel,
        createValue,
    ...
    const getOptionDisabled = (0, react_1.useCallback)(option => {
    ...
    const getOptionLabel = (0, react_1.useCallback)((option, isListItem = false) => {
        // eslint-disable-next-line eqeqeq
        if (option == undefined) {
            return '';
        }
        ...
        if (option?.id === createId) {
            return (0, get_js_1.default)(option, typeof optionText === 'string' ? optionText : 'name');
        }
    ...
    const handleInputChange = (0, ra_core_1.useEvent)((event, newInputValue, reason) => {
        if (event?.type === 'change' ||
            !doesQueryMatchSelection(newInputValue)) {
            const createOptionLabel = typeof createItemLabel === 'string'
    ...
    const doesQueryMatchSelection = (0, react_1.useCallback)((filter) => {
    ...
    const doesQueryMatchSuggestion = (0, react_1.useCallback)(filter => {
    ...
    const handleAutocompleteChange = (0, react_1.useCallback)((event, newValue, reason) => {
    ...
    const oneSecondHasPassed = (0, ra_core_1.useTimeout)(1000, filterValue);
    ...
    const handleInputRef = (0, material_1.useForkRef)(field.ref, TextFieldProps?.inputRef);
    ...
    const finalLoadingText = typeof loadingText === 'string'
        ? translate(loadingText, {
            _: loadingText,
        })
        : loadingText;
    return (React.createElement(React.Fragment, null,
        React.createElement(StyledAutocomplete, { className: (0, clsx_1.default)('ra-input', `ra-input-${source}`, className), clearText: translate(clearText, { _: clearText }), closeText: translate(closeText, { _: closeText }), loadingText: isPaused && isPlaceholderData
                ? offline !== false && offline !== undefined
                    ? offline
                    : finalLoadingText
                : finalLoadingText, openOnFocus: true, openText: translate(openText, { _: openText }), id: id, isOptionEqualToValue: isOptionEqualToValue, filterSelectedOptions: true, disabled: disabled || readOnly, renderInput: params => {
                ...
                const mergedSlotProps = TextFieldProps?.slotProps
                    ...
                    : undefined;
                return (React.createElement(material_1.TextField, { name: field.name, label: label !== '' && label !== false ? (React.createElement(ra_core_1.FieldTitle, { label: label, source: source, resource: resourceProp, isRequired: isRequired })) : null, error: !!fetchError || invalid, helperText: renderHelperText ? (React.createElement(InputHelperText_1.InputHelperText, { error: error?.message ||
                            fetchError?.message, helperText: helperText })) : null, margin: margin, variant: variant, className: (0, clsx_1.default)({
                        [exports.AutocompleteInputClasses.textField]: true,
                        [exports.AutocompleteInputClasses.emptyLabel]: label === false || label === '',
                    }), ...params, ...TextFieldProps, InputProps: mergedTextFieldProps, ...mergedSlotProps, size: size, inputRef: handleInputRef }));
            }, multiple: multiple, renderTags: (value, getTagProps) => value.map((option, index) => {
                ...
                const mergedSlotProps = props.slotProps?.chip
                    ...
                    : props.ChipProps;
                return (React.createElement(material_1.Chip, { label: (0, react_1.isValidElement)(optionText)
                        ? inputText
                            ? inputText(option)
                            : ''
                ...
                : noOptionsText, selectOnFocus: true, clearOnBlur: clearOnBlur, ...(0, sanitizeInputRestProps_1.sanitizeInputRestProps)(rest), freeSolo: !!create || !!onCreate, open: isOpen && canRenderSuggestions, onOpen: handleOpen, onClose: handleClose, handleHomeEndKeys: !!create || !!onCreate, filterOptions: filterOptions, options: isPaused && isPlaceholderData
                ? []
                : canRenderSuggestions
                    ? suggestions
                    : [], getOptionKey: (option) => option?.id, getOptionLabel: getOptionLabelString, inputValue: filterValue, loading: (isPending &&
                (!finalChoices || finalChoices.length === 0) &&
                oneSecondHasPassed) ||
                (isPaused && isPlaceholderData), value: selectedChoice, onChange: handleAutocompleteChange, onBlur: finalOnBlur, onInputChange: handleInputChange, renderOption: (props, record) => {
                // We have to extract the key because react 19 does not allow to spread the key prop
                ...
                return (React.createElement("li", { key: key, ...rest }, optionLabel === '' ? ' ' : optionLabel));
...
const StyledAutocomplete = (0, styles_1.styled)(material_1.Autocomplete, {
    name: PREFIX,
    overridesResolver: (props, styles) => styles.root,
})(({ theme }) => ({
    [`& .${exports.AutocompleteInputClasses.textField}`]: {
        minWidth: theme.spacing(20),
    },
    [`& .${exports.AutocompleteInputClasses.emptyLabel} .MuiOutlinedInput-root legend`]: {
        width: 0,
    },
}));
/**
...
const useSelectedChoice = (value, { choices, multiple, optionValue, }) => {
    const selectedChoiceRef = (0, react_1.useRef)(getSelectedItems(choices, value, optionValue, multiple));
    const [selectedChoice, setSelectedChoice] = (0, react_1.useState)(() => getSelectedItems(choices, value, optionValue, multiple));
...
const getSelectedItems = (choices = [], value, optionValue = 'id', multiple) => {
    if (multiple) {
        return (Array.isArray(value ?? []) ? value : [value])
            .map(item => choices.find(choice => String(item) === String((0, get_js_1.default)(choice, optionValue))))
            .filter(item => !!item);
    }
    return (choices.find(choice => String((0, get_js_1.default)(choice, optionValue)) === String(value)) || '');
};
const areSelectedItemsEqual = (selectedChoice, newSelectedChoice, optionValue = 'id', multiple) => {
    if (multiple) {
        ...
        const equalityArray = selectedChoiceArray.map(choice => newSelectedChoiceArray.some(newChoice => (0, get_js_1.default)(newChoice, optionValue) === (0, get_js_1.default)(choice, optionValue)));
        return !equalityArray.some(item => item === false);
    }
    return ((0, get_js_1.default)(selectedChoice, optionValue) === (0, get_js_1.default)(newSelectedChoice, optionValue));
};
const DefaultFilterToQuery = searchText => ({ q: searchText });
const defaultOffline = React.createElement(Offline_1.Offline, { variant: "inline" });
